...
| **Audit API** (`api-audit.kai-studio.ai`)         | 1) `audit-ui` (browser, cookie auth) — 2) MCP clients (Claude Desktop/Code, Cursor, Le Chat) driving the conflict-first workflow from an LLM | ✅ 19 curated tools **BETA** (40 REST endpoints) | Multi-step workflow → LLM can orchestrate conflict resolution via MCP. Human user delegating to an agent → OAuth 2.1.      |
